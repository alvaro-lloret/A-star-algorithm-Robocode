...
public class RouteFinder {
	...
	public static void main(String[] args) {
		...
		boolean hideEnemyNames = false; 
		   ...
		   *  so that no pair of obstacles
		   * are at the same position     **/
		RobotSpecification[] modelRobots = 
				engine.getLocalRepository ("sample.SittingDuck,searchpractice.RouteBot*");
		 
		RobotSpecification[] existingRobots = new RobotSpecification[NumObstacles+1];          RobotSetup[] robotSetups = new RobotSetup[NumObstacles+1]; 
		...
		for(int NdxObstacle=0;NdxObstacle<NumObstacles;NdxObstacle++){
			...
			robotSetups[NdxObstacle]=
					  new RobotSetup(InitialObstacleRow,InitialObstacleCol,0.0);                 
		}
		...
		/** Create the agent and place it in a random position without obstacle **/
		 ...
		 InitialAgentRow=...;   
		 InitialAgentCol=...;   
		 robotSetups[NumObstacles]=
				 new RobotSetup(InitialAgentRow,InitialAgentCol,0.0); 
		 
		 /** Create and run the battle **/ 
		 	
		 BattleSpecification battleSpec = 
				 new BattleSpecification(battlefield,
				 numberOfRounds, inactivityTime, gunCoolingRate,
				 sentryBorderSize, hideEnemyNames, existingRobots,
				 robotSetups);
